...
fun DietScreen(
    ...
    val favorites by viewModel.favoriteDiets.collectAsStateWithLifecycle()
    ...
    Column(
        ...
        CommonTopBar(title = "식단 기록", onBackClick = onBackHomeClick)
        OutlinedTextField(
            value = date,
        ...
        if (favorites.isNotEmpty()) {
            Text(text = "즐겨찾기", style = MaterialTheme.typography.titleMedium)
            LazyColumn(
                modifier = Modifier.weight(0.35f, fill = false),
                verticalArrangement = Arrangement.spacedBy(8.dp)
            ) {
                items(favorites, key = { "favorite-${it.id}" }) { diet ->
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                            ...
                            Text(text = "${diet.foodName} / ${diet.calorie} kcal")
                            Button(onClick = { viewModel.addFromFavorite(diet) }) {
                                Text(text = "추가")
                            }
                        }
                    }
        ...
        Text(text = "선택 날짜 식단", style = MaterialTheme.typography.titleMedium)
        if (diets.isEmpty()) {
            Text(text = "선택한 날짜에 등록된 식단 기록이 없습니다.")
        } else {
            LazyColumn(
                        ...
                        Column(
                            ...
                            Row(
                                ...
                                Column(modifier = Modifier.weight(1f)) {
                                    Text(text = diet.foodName, style = MaterialTheme.typography.titleMedium)
