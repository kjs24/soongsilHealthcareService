...
fun HomeScreen(
    ...
    Column(
        ...
        Text(text = "${summary.nickname}님의 ${DateUtil.today()} 건강 기록")
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(
                modifier = Modifier.padding(16.dp),
                verticalArrangement = Arrangement.spacedBy(8.dp)
            ) {
                Text(text = "운동 소모 칼로리: ${summary.exerciseCalorie} kcal")
                Text(text = "식단 섭취 칼로리: ${summary.dietCalorie} kcal")
                Text(text = "균형: ${summary.dietCalorie - summary.exerciseCalorie} kcal")
            }
        }
